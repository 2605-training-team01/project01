...
def render():
    ...
    try:
        ...
        tab_day, tab_month, tab_year, tab_search, tab_member = st.tabs([
            "일별", "월별", "연도별", "기간검색", "회원조회"
        ])

        # 공통 함수: 데이터프레임 형식 변환 및 예외 처리
        def format_sales_df(data, columns, rename_dict=None):
            if not data:
            ...
            # 매출액 포맷팅 (데이터가 존재하는 경우)
            if "매출액" in df.columns:
                df["매출액"] = df["매출액"].fillna(0).astype(int).apply(lambda x: f"{x:,}원")
            elif "결제금액" in df.columns:
                df["결제금액"] = df["결제금액"].fillna(0).astype(int).apply(lambda x: f"{x:,}원")
